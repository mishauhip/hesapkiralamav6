 ...
 *  Not: Bu sayfa tek dosya olsun diye buraya taşındı.
 *  ---------------------------------------------------------------- */
...
/**
 * account-v1 + match-v5 için "regional routing": americas / europe / asia / sea
 * summoner-v4 + league-v4 için "platform routing": tr1 / euw1 / na1 / kr / ...
 */
type PlatformRoute =
  | "tr1"
  | "euw1"
...
export async function getSummonerByPUUID(platform: PlatformRoute, puuid: string) {
  return fetchJson(`${BASE}/summoner/${platform}/${enc(puuid)}`);
}

export async function getRankedInfo(platform: PlatformRoute, summonerId: string) {
...
export async function getMatchDetails(regional: RegionalRoute, matchId: string) {
...
export default function AccountDetailPage() {
  ...
  const handleReleaseAccount = async () => {
    try {
      ...
      // App Router'da router.refresh her zaman olmayabilir; fallback bırakıyoruz.
      try {
        // @ts-ignore
        router.refresh?.();
      } catch {}
          ...
          const enriched = await Promise.all(
            (assignmentsData as AccountAssignment[]).map(async (assignment) => {
              ...
              return {
                ...
                initial_league: assignment.initial_league || "Bilinmiyor",
                initial_flex_league: assignment.initial_flex_league || "Unranked",
                initial_solo_lp: assignment.initial_solo_lp ?? 0,
                initial_flex_lp: assignment.initial_flex_lp ?? 0,
              };
            })
          );
                ...
                <AlertDialog open={showReleaseDialog} onOpenChange={setShowReleaseDialog}>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={handleReleaseAccount} disabled={releaseLoading}>
          ...
          <CardContent>
            <div className="space-y-4">
              {account.matches.map((match) => {
                const participant =
                  match.info.participants.find((p) => p.puuid === account.summoner?.puuid) ??
                  (() => {
                ...
                const gameDurationMin = Math.max(1, Math.floor(match.info.gameDuration / 60));
