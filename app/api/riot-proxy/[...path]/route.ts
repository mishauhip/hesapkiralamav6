...
export async function GET(
    ...
    const seg = (ctx.params?.path ?? []).map(String);
    ...
    const continent = PLAT_TO_CONT[platform] ?? platform; // americas/europe/asia/sea gelirse aynen kullan
    const qs = req.nextUrl.search ?? "";

    let target = "";

    if (kind === "account-by-riotid") {
        // BU endpoint continent domain ister
        ...
        const puuid = seg[2];
        if (!puuid) return NextResponse.json({ error: "missing puuid" }, { status: 400 });
        target = `https://${continent}.api.riotgames.com/lol/match/v5/matches/by-puuid/${encodeURIComponent(
            puuid
        )}${qs}`;
    } else if (kind === "match") {
        const matchId = seg[2];
        if (!matchId) return NextResponse.json({ error: "missing matchId" }, { status: 400 });
        target = `https://${continent}.api.riotgames.com/lol/match/v5/matches/${encodeURIComponent(
            matchId
        )}`;
    }
