...
type RiotProxyParams = { path: string[] };
type RiotProxyCtx = { params: Promise<RiotProxyParams> };

export async function GET(req: NextRequest, ctx: RiotProxyCtx) {
  const apiKey = process.env.RIOT_API_KEY || "";
  if (!apiKey) {
    return NextResponse.json({ error: "RIOT_API_KEY missing" }, { status: 500 });
  }

  // Next 15: params async (build type check bunu istiyor)
